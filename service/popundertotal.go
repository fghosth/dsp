...
type popundertotalDSPBidder struct {
	logger *log.Logger
	lock   *sync.RWMutex
}
...
/*
	 ...
	 @return []byte 索引数据
*/
func (s popundertotalDSPBidder) SyncIndex() []byte {
...
*/
func (s popundertotalDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s popundertotalDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewPopundertotalADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s popundertotalDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *popundertotalDSPBidder) finish() {
...
func (s *popundertotalDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *popundertotalDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewPopundertotalServer(logger *log.Logger) *popundertotalDSPBidder {
	sb := &popundertotalDSPBidder{logger, new(sync.RWMutex)}
