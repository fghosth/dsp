...
/*
	 ...
	 @return []byte 索引数据
*/
func (s adsterraDSPBidder) SyncIndex() []byte {
...
*/
func (s adsterraDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s adsterraDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewAdsterraADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s adsterraDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *adsterraDSPBidder) finish() {
...
func (s *adsterraDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *adsterraDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewAdsterraServer(logger *log.Logger) *adsterraDSPBidder {
	sb := &adsterraDSPBidder{logger, new(sync.RWMutex)}
