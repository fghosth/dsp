...
type hilltopadsDSPBidder struct {
	logger *log.Logger
	lock   *sync.RWMutex
}
...
/*
	 ...
	 @return []byte 索引数据
*/
func (s hilltopadsDSPBidder) SyncIndex() []byte {
...
*/
func (s hilltopadsDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s hilltopadsDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewHilltopadsADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s hilltopadsDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *hilltopadsDSPBidder) finish() {
...
func (s *hilltopadsDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *hilltopadsDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewHilltopadsServer(logger *log.Logger) *hilltopadsDSPBidder {
	sb := &hilltopadsDSPBidder{logger, new(sync.RWMutex)}
