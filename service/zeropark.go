...
/*
	 ...
	 @return []byte 索引数据
*/
func (s zeroparkDSPBidder) SyncIndex() []byte {
...
*/
func (s zeroparkDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s zeroparkDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewZeroparkADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s zeroparkDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *zeroparkDSPBidder) finish() {
...
func (s *zeroparkDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *zeroparkDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewZeroparkServer(logger *log.Logger) *zeroparkDSPBidder {
	sb := &zeroparkDSPBidder{logger, new(sync.RWMutex)}
