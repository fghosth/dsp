...
/*
	 ...
	 @return []byte 索引数据
*/
func (s popcashDSPBidder) SyncIndex() []byte {
...
*/
func (s popcashDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s popcashDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewPOPCASHADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s popcashDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *popcashDSPBidder) finish() {
...
func (s *popcashDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *popcashDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewPopcashServer(logger *log.Logger) *popcashDSPBidder {
	sb := &popcashDSPBidder{logger, new(sync.RWMutex)}
