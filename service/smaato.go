...
/*
	 ...
	 @return []byte 索引数据
*/
func (s smaatoDSPBidder) SyncIndex() []byte {
...
*/
func (s smaatoDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s smaatoDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewSmaatoADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s smaatoDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *smaatoDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *smaatoDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewSmaatoServer(logger *log.Logger) *smaatoDSPBidder {
	sb := &smaatoDSPBidder{logger, new(sync.RWMutex)}
	return sb
}
