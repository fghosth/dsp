...
/*
	 ...
	 @return []byte 索引数据
*/
func (s mgidDSPBidder) SyncIndex() []byte {
...
*/
func (s mgidDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s mgidDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewMGIDADX(nil, config.ADXCode[config.ADX], host)
	offer.SetData(body)
	offer.SetOfferInfo()
...
/*
	...
	@return error
*/
func (s mgidDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *mgidDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *mgidDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewMGIDServer(logger *log.Logger) *mgidDSPBidder {
	sb := &mgidDSPBidder{logger, new(sync.RWMutex)}
