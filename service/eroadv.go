...
/*
	 ...
	 @return []byte 索引数据
*/
func (s EroadvSPBidder) SyncIndex() []byte {
...
*/
func (s EroadvSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s EroadvSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewEroadvADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s EroadvSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *EroadvSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *EroadvSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewEroadvServer(logger *log.Logger) *EroadvSPBidder {
	sb := &EroadvSPBidder{logger, new(sync.RWMutex)}
