...
type propelleradsDSPBidder struct {
	logger *log.Logger
	lock   *sync.RWMutex
}
...
/*
	 ...
	 @return []byte 索引数据
*/
func (s propelleradsDSPBidder) SyncIndex() []byte {
...
*/
func (s propelleradsDSPBidder) StopBidByCID(cid uint32) bool {
	var res bool
...
/*
   ...
   @return error
*/
func (s propelleradsDSPBidder) Bidder(body []byte, host string) string {
	defer func() {
		if err := recover(); err != nil {
			pp.Println("bidder意外崩溃", err) // 这里的err其实就是panic传入的内容，55
		}
	}()
	offer := model.NewPropelleradsADX(body, config.ADXCode[config.ADX], host)
...
/*
	...
	@return error
*/
func (s propelleradsDSPBidder) ADXNotify(notify rabbitmq.WinNotify) error {
...
func (s *propelleradsDSPBidder) finish() {
...
func (s *propelleradsDSPBidder) job(job Jobs) interface{} {
	var cid uint32 = 0
...
func (s *propelleradsDSPBidder) dealres(obj interface{}, compres *CompaignResult) (err error) {
...
@return  struct
*/
func NewPropelleradsServer(logger *log.Logger) *propelleradsDSPBidder {
	sb := &propelleradsDSPBidder{logger, new(sync.RWMutex)}
